...
def merge_training_results(latest_metrics: Dict[str, Any], training_results: Optional[Dict[str, Any]], logger=None) -> Dict[str, Any]:
    ...
    if training_results:
        if logger:
            logger.info(f"Merging training_results into latest_metrics: {training_results}")
        for key, value in training_results.items():
            if value is not None:
                latest_metrics[key] = str(value)
        if (
            "final_loss" in training_results
            and ("best_accuracy" not in latest_metrics or latest_metrics["best_accuracy"] == "N/A")
        ):
            try:
...
def determine_model_type(config: Dict[str, Any], logger=None) -> Optional[str]:
    ...
    is_autoencoder = any(pattern in model_name or pattern in model_wrapper 
    ...
    if os.path.exists(dataset_config_path):
        try:
            with open(dataset_config_path, 'r') as f:
                ...
                if logger:
                    logger.info(f"Extracted dataset metadata from {dataset_config_path}")
        except Exception as e:
            if logger:
...
def initialize_metadata_structure(experiment_metadata: Dict[str, Any], has_train: bool, has_inference: bool) -> Dict[str, Any]:
...
def _add_proxy_accuracy_if_needed(latest_metrics: Dict[str, Any], training_results: Optional[Dict[str, Any]]) -> None:
    if training_results and (
        "final_loss" in training_results and ("best_accuracy" not in latest_metrics or latest_metrics["best_accuracy"] == "N/A")
    ):
        try:
            final_loss = float(training_results["final_loss"])
            if final_loss <= 10.0:
...
def _normalize_training_results(training_results: Optional[dict]) -> Optional[dict]:
    ...
    if training_results is None:
        return None
    if not isinstance(training_results, dict) and hasattr(training_results, "summary") and callable(training_results.summary):
...
def _merge_metrics_dicts(latest_metrics: Dict[str, Any], training_results: Optional[Dict[str, Any]]) -> Dict[str, Any]:
...
def _filter_recent_log_files(log_files_with_time: List[Tuple[str, float]], max_age_seconds: int = 86400) -> List[Tuple[str, float]]:
...
def _extract_xai_viz_info(config_data: Dict[str, Any]) -> tuple:
    ...
    if 'runtime' in config_data and 'hooks' in config_data['runtime']:
        ...
        # Extract XAI methods and layer information
        ...
        for config in explainability_configs:
            if isinstance(config, dict) and 'method' in config:
                ...
                if 'params' in config:
                    ...
                    # Store all method-specific configurations
        ...
        # Add layer info if any layer information was found
        if layer_info:
            xai_info['layer_info'] = layer_info
            
        viz_info = {
            'methods': hooks.get('visualizations', [])
        }
...
def extract_run_metadata(config_files: List[str], checkpoints_dir: str) -> Dict[str, Any]:
    """Extract run metadata such as file lists, including from explanations directory."""
    # Check both checkpoints_dir and explanations directory
    npy_files = []
    png_files = []
    
    # First, look in checkpoints_dir
    npy_files.extend(glob.glob(os.path.join(checkpoints_dir, '**', '*.npy'), recursive=True))
    png_files.extend(glob.glob(os.path.join(checkpoints_dir, '**', '*.png'), recursive=True))
    ...
    if os.path.exists(explanations_dir):
        npy_files.extend(glob.glob(os.path.join(explanations_dir, '**', '*.npy'), recursive=True))
        png_files.extend(glob.glob(os.path.join(explanations_dir, '**', '*.png'), recursive=True))
    ...
    if os.path.exists(global_explanations_dir):
        npy_files.extend(glob.glob(os.path.join(global_explanations_dir, '**', '*.npy'), recursive=True))
        png_files.extend(glob.glob(os.path.join(global_explanations_dir, '**', '*.png'), recursive=True))
    
    # Extract experiment ID from checkpoints_dir for filtering
    experiment_id = os.path.basename(checkpoints_dir).split('_')[-1] if '_' in os.path.basename(checkpoints_dir) else "unknown"
    
    # Filter files by experiment ID using the same logic as collect_run_metadata
    def filter_by_experiment(files, exp_id):
        experiment_patterns = [
            f"{exp_id}/train",
            f"{exp_id}/inference", 
...
def fix_experiment_info_structure(experiment_metadata: Dict[str, Any], has_train: bool, has_inference: bool) -> Dict[str, Any]:
    ...
    if isinstance(experiment_id, dict):
        experiment_id = experiment_id.get('experiment_id') or experiment_id.get('experiment_dir', '').split('_')[-1]
...
def _extract_metrics_from_log_lines_helper(lines: List[str], metrics: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for file in fs:
            for ext in extensions:
                if file.endswith(ext):
                    rel_path = os.path.relpath(os.path.join(root, file), explanations_dir)
...
def extract_run_files_metadata(explanations_dir: str, experiment_id: str) -> Dict[str, List[str]]:
