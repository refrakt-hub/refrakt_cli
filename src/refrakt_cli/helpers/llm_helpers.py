...
def setup_llm_environment(logger=None) -> Optional[str]:
    """
...
def _get_experiment_directories(explanations_dir: str, logger=None) -> List[Tuple[str, float]]:
    """
...
def find_latest_experiment_dir(explanations_dir: str, logger=None) -> Optional[str]:
    """
...
def generate_and_save_explanations(exp_dir: str, logger=None):
    """
    ...
    xai_files_by_method = organize_xai_files_by_method(npy_files + png_files, logger=logger)
    ...
    if os.path.exists(summary_metrics_path):
        try:
            ...
            # Fallback to extracting metadata from config files
