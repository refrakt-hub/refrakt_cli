...
def setup_llm_environment(logger: Optional[logging.Logger] = None) -> Optional[str]:
    ...
    if not os.path.exists(explanations_dir):
        if logger:
            logger.warning("No explanations directory found. Skipping LLM explanations.")
        return None
...
def _get_experiment_directories(explanations_dir: str, logger: Optional[logging.Logger] = None) -> List[Tuple[str, float]]:
    """
...
def find_latest_experiment_dir(explanations_dir: str, logger: Optional[logging.Logger] = None) -> Optional[str]:
    """
    ...
    if not experiment_dirs:
        if logger:
            logger.warning("No experiment directories with explanations found in explanations/. Skipping LLM explanations.")
        return None
...
def generate_and_save_explanations(exp_dir: str, logger: Optional[logging.Logger] = None) -> None:
    """
    ...
    from refrakt_cli.utils.llm_utils import organize_xai_files, generate_method_explanation
    ...
    # Debug output
    if logger:
        logger.info(f"DEBUG: Found {len(npy_files)} npy files and {len(png_files)} png files")
        ...
        for method_name, method_data in xai_files_by_method.items():
            logger.info(f"DEBUG:   {method_name}: {len(method_data.get('files', []))} files")

    # Read metadata from summary_metrics.json
    exp_name = os.path.basename(exp_dir)
    checkpoints_dir = os.path.join(os.path.dirname(os.path.dirname(exp_dir)), 'checkpoints')
    summary_metrics_path = os.path.join(checkpoints_dir, exp_name, 'explanations', 'summary_metrics.json')
    ...
    if os.path.exists(summary_metrics_path):
        try:
            ...
            if logger:
                logger.warning(f"Failed to load metadata from {summary_metrics_path}: {e}")
            ...
            metadata = extract_comprehensive_metadata(config_files, base_dir, checkpoints_dir, logger=logger)
    else:
        if logger:
            logger.warning(f"summary_metrics.json not found at {summary_metrics_path}, falling back to config file extraction")
        # TODO: Move extract_comprehensive_metadata to utils during Phase 4
        from refrakt_cli.helpers.shared_core import extract_comprehensive_metadata
        base_dir = os.path.dirname(exp_dir)
        metadata = extract_comprehensive_metadata(config_files, base_dir, checkpoints_dir, logger=logger)
    ...
    for method_name, method_files in xai_files_by_method.items():
        explanation = generate_method_explanation(
            method_name, method_files, metadata, config_files, system_prompt, model_name, logger=logger
        )
